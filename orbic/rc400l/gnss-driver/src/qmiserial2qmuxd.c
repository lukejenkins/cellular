 ...
 *
 * Modified for Orbic RC400L (MDM9207) - changed socket path from
 * Android /dev/socket/qmux_radio/ to /var/ for non-Android Qualcomm Linux.
 *
...
/* Orbic RC400L uses /var/qmux_connect_socket (non-Android Qualcomm Linux) */
#ifndef SOCKPATH
#define SOCKPATH "/var/"
#endif
...
err_t handle_qmuxd_response(const struct qmuxd_hdr *qmuxd_hdr, const void *msg) {
	LOG("[%"PRIu16", syserr=%"PRId32", qmierr=%"PRIu32", length=%zu]> %"PRIu32" svc=%"PRIu8,
		qmuxd_hdr->transaction, qmuxd_hdr->sys_err, qmuxd_hdr->qmi_err,
		(size_t)(qmuxd_hdr->len - sizeof(*qmuxd_hdr)),
...
bool_t writevall(int fd, const struct iovec *iov, int iovcnt) {
	...
	if (ret < 0) return 0;
	if ((size_t)ret != total) { errno = EAGAIN; return 0; }
	return 1;
}
...
err_t read_msg(int fd, bool_t expect_frame, void *msg, size_t msg_buf_size) {
	...
	if (len > msg_buf_size) {
		LOG("message too big for buffer (%"PRIu16" > %zu)", len, msg_buf_size);
		return EOVERFLOW;
	}
	if (len > sizeof(*msg_ui16)) {
...
err_t open_qmuxd_socket() {
	...
	LOG("connected to qmuxd, client id %"PRIu32, g_qmuxd_client_id);
	return 0;
...
int main(__unused int argc, __unused char *argv[]) {
	...
	if ((err = open_qmuxd_socket())) {
		LOG("error opening qmux: %s", strerror(err));
		return 1;
	}
	...
	if ((err = pthread_create(&socket_read_thread, NULL, qmuxd_read_loop, NULL))) {
		LOG("error creating thread: %s", strerror(err));
		return 1;
	}
